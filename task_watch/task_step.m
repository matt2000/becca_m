...
if task.sample_ctr == 0
    task.col_pos = ceil( rand(1) * ...
        (task.col_max - task.col_min)) + task.col_min;
    task.row_pos = ceil( rand(1) * ...
        (task.row_max - task.row_min)) + task.row_min;
end

task.sample_ctr = task.sample_ctr + 1;

if task.sample_ctr >= task.TASK_DURATION
    task.sample_ctr = 0;
...
task.fov = task.data(   task.row_pos - floor( task.fov_hgt/2): ...
